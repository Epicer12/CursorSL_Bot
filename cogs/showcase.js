...
const shipCommand = {
  ...
  async slashExecute(interaction, { db, config, logger }) {
    ...
    const description = interaction.options.getString("description") || "A new project joined the CursorSL showcase!";
    const image = interaction.options.getAttachment("image");
    const userId = interaction.user.id;

    if (!config.showcaseChannelId) {
      await interaction.reply({ content: "Showcase channel is not configured. Please contact an admin.", ephemeral: true });
      return;
    }

    try {
      const channel = await interaction.client.channels.fetch(config.showcaseChannelId);
      if (!channel) {
        await interaction.reply({ content: "Showcase channel not found.", ephemeral: true });
        return;
      }
      ...
      await interaction.editReply(`Congratulations! Your project **${title}** has been shipped to <#${config.showcaseChannelId}>!`);
    } catch (error) {
      logger.error("Failed to ship project", { error: error.message });
      if (interaction.deferred) {
        await interaction.editReply("Something went wrong while shipping your project.");
      } else {
        await interaction.reply({ content: "Something went wrong while shipping your project.", ephemeral: true });
      }
    }
  },
};
