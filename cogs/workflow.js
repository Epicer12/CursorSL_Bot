  ...
  slashData: new SlashCommandBuilder()
    ...
    .addStringOption(option =>
      option.setName("title")
        .setDescription("Title of your workflow (e.g., 'React + Tailwind Rules')")
        .setRequired(true))
    .addStringOption(option =>
  ...
  async slashExecute(interaction, { db, config, logger }) {
    ...
    try {
      ...
      if (config.workflowChannelId) {
        const channel = await interaction.client.channels.fetch(config.workflowChannelId);
        if (channel) {
          const publicEmbed = new EmbedBuilder()
            ...
            .addFields(
              ...
              { name: "Content Preview", value: `\`\`\`${content.slice(0, 500)}${content.length > 500 ? "..." : ""}\`\`\`` }
            )
            .setFooter({ text: `Use /workflow id:${result.lastID} to see full details` })
            .setColor("#00ff99");
      ...
      await interaction.reply({ content: "Failed to save your workflow. Please try again.", ephemeral: true });
    }
  },
};
...
const workflowsCommand = {
  ...
  async slashExecute(interaction, { db }) {
    const rows = await db.all("SELECT id, title, user_id FROM workflows ORDER BY created_at DESC LIMIT 10");

    if (rows.length === 0) {
      await interaction.reply("No workflows shared yet. Be the first with `/share-workflow`!");
      return;
    }

    const embed = new EmbedBuilder()
      .setTitle("📚 Community Workflows")
      .setDescription(rows.map(r => `**#${r.id}** - ${r.title} (by <@${r.user_id}>)`).join("\n"))
      .setColor("#0099ff");
